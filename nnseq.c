...
void nn_long_seq (int dimensions, int trcount, long long *trdata, int *trklass,
                  int count, long long *data, int *klass)
{
        int n, tn;
        int i, ti;
        int cl, d, idx;
        long long min_distance, dist;
        long long tmp, mask;
        ...
        for (n = 0;  n < count;  n++)
        {
                ...
                for (tn = 0;  tn < trcount;  tn++)
                {
                        ...
                        {
                                tmp = data[i + d] - trdata[i + d];
                                mask = tmp >> (sizeof(long long) * CHAR_BIT - 1);
                                dist += (tmp ^ mask) - mask;
                        }
                        if (dist < min_distance)
                        {
